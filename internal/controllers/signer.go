...
type HealthCheckerBuilder func(*omegaissuerapi.IssuerSpec, map[string][]byte) (HealthChecker, error)

type Signer interface {
	Sign([]byte) ([]byte, error)
}

type SignerBuilder func(*omegaissuerapi.IssuerSpec, map[string][]byte) (Signer, error)
...
func (s Issuer) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	return (&controllers.CombinedController{
		IssuerTypes:        []issuerapi.Issuer{&omegaissuerapi.OmegaIssuer{}},
		ClusterIssuerTypes: []issuerapi.Issuer{&omegaissuerapi.OmegaClusterIssuer{}},

		FieldOwner:       "omegaissuer.cert-manager.io",
		MaxRetryDuration: 1 * time.Minute,

		Sign:          s.Sign,
		Check:         s.Check,
		EventRecorder: mgr.GetEventRecorderFor("omegaissuer.cert-manager.io"),
	}).SetupWithManager(ctx, mgr)
}

func (o *Issuer) getIssuerDetails(issuerObject issuerapi.Issuer) (*omegaissuerapi.IssuerSpec, string, error) {
	switch t := issuerObject.(type) {
	case *omegaissuerapi.OmegaIssuer:
		return &t.Spec, issuerObject.GetNamespace(), nil
	case *omegaissuerapi.OmegaClusterIssuer:
...
func (o *Issuer) Sign(ctx context.Context, cr signer.CertificateRequestObject, issuerObject issuerapi.Issuer) (signer.PEMBundle, error) {
	...
	signed, err := signerObj.Sign(certDetails.CSR)
	if err != nil {
